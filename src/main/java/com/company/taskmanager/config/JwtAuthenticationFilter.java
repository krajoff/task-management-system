...
/**
 * JwtAuthenticationFilter — это фильтр, который обрабатывает каждый входящий HTTP-запрос,
 * извлекая и проверяя JWT-токен из заголовка Authorization. Если токен валиден, фильтр
 * аутентифицирует пользователя и устанавливает контекст безопасности для дальнейшей обработки.
 *
 * <p>Этот фильтр является частью цепочки фильтров Spring Security и выполняется один раз за запрос.
 * Он использует {@link JwtService} для обработки токена (его извлечения и проверки) и {@link UserService}
 * для загрузки данных пользователя.</p>
 *
 * <p>Если токен отсутствует или недействителен, фильтр передаёт запрос дальше по цепочке,
 * не изменяя контекст безопасности. Если токен валиден, он аутентифицирует пользователя и
 * устанавливает аутентификацию в {@link SecurityContextHolder}.</p>
 */
@Component
